...
def hex_plot(ph1_clean, th1_clean, vel1_clean, ax=None, nx=20, vmin=400, vmax=800, do_print_top=True, do_hex=False):
    ...
    ph1 = ph1_wrap = (ph1_clean)%(2*np.pi)
    th1 = th1_clean 
    vel1 = vel1_clean
    ...
    nx = np.min((np.max((nx, 10)), 20))

    # print("NX = ", nx, "\n\n\n\n\n\n\n")

    gridsize = (int(nx), int(np.round(nx*ratio)))
    ...
    if do_hex:
        ...
        # fig, ax = plt.subplots()
        contour1 = hex_ax.imshow(z1_use, extent=(0, 2*np.pi, -1, 1), cmap="autumn", 
                                 vmin=vmin, vmax=vmax, zorder=10000, origin='lower', interpolation='nearest')
...
if __name__ == "__main__":
    ...
    # hex_plot(ph1_clean, th1_clean, vel1_clean, ax=contour_ax, nx=20, vmin=all_vmin, vmax=all_vmax, do_hex=False)
    ...
    # contour_ax.scatter(ph1b, th1b,           color=(drk, drk, drk), s=3**2, alpha=1, marker='+')
    
    scatter_ax.scatter(ph1_clean, th1_clean, c='k', s=2**2, alpha=1., marker='o')
    # hex_ax.scatter(ph1_clean, th1_clean, c='k',     s=2**2, alpha=1., marker='o')
    # contour_ax.scatter(ph1_clean, th1_clean, c='k', s=2**2, alpha=1., marker='o')
