...
def magnet_plot(get_cr, datdir, batch, open_f=None, closed_f=None, force=False, reduce=0, nact=0, nwant=None, do_print_top=False,
    ...
    if np.max(np.abs(f_lat)) > 1.0:
        f_lat = np.deg2rad(f_lat)
    # import pdb; pdb.set_trace()
...
if __name__ == "__main__":
    ...
    # import pdb; pdb.set_trace()
    CR = args.cr or cr
    n_open, n_closed, n_flux, fnum, n_outliers = magnet_plot(CR, args.dat_dir, batch, do_print=True, reduce=reduce, nwant=args.nwant, do_print_top=True)
