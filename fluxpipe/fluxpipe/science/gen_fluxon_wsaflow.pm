...
our @EXPORT_OK = qw(gen_fluxon_wsaflow);
...
sub gen_fluxon_wsaflow {
    ...
    (my $r_vr_scaled, my $r_fr_scaled, my $theta, my $phi) =
            gen_fluxon_wsaflow_phys($fluxon, $ch_map_path);
...
sub gen_fluxon_wsaflow_phys {
    ...
    our $fr = pdl(($r0*$r0)/($r*$r) * $bmag->((1))/$bmag);
    ...
    # plot_image_with_points($ch_map_path, [$th0_ind, $ph0_ind], [$theta_bound, $phi_bound]);


    my $flow_field = wind_speed($fr, $distance);

    # print "\n\n\nfr = $fr\n\n";
    # print "f1 = $fr1\n\n";
    # print "Theta0 = $th0_ind, Phi0 = $ph0_ind\n\n";
    # print "Theta1 = $theta_bound, Phi1 = $phi_bound, Distance = $distance\n\n";
...
sub gradient_descent {
    ...
    my $col = ($longitude); # If not, you should uncomment the scaling
...
sub plot_image_with_points {
    my ($image_path, $original_point, $discovered_point) = @_;

    # Save data and image path to a CSV file
    open my $fh, '>', 'data.csv' or die "Cannot open data.csv: $!";
    print $fh "type,x,y,image_path\n";
    print $fh "original,$original_point->[0],$original_point->[1],$image_path\n";
    print $fh "discovered,$discovered_point->[0],$discovered_point->[1],$image_path\n";
    close $fh;
