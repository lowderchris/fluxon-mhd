...
AV *
_closest_nsimplex(wsv,xsv,gsv=&PL_sv_undef,vsv=&PL_sv_undef)
 SV *wsv
 SV *xsv
 SV *gsv
 SV *vsv
PREINIT:
 /**********************************************************************
  * closest_nsimplex -- return a list of VERTEXes describing a simplex
  * close to the target point, enclosing the point in a 'nice' manner.
  */
  WORLD *w;
  VERTEX *v;
  POINT3D x;
  ...
  int i;
CODE:
  w = SvWorld(wsv,"closest_nsimplex",1);
  ...
  global = SvIV(gsv);
  dl = FLUX->find_nsimplex_by_location(x, w, v, global);
 ...
 **** modules and core functions.   **/
 ...
 if(CoreSV==NULL)     Perl_croak(aTHX_ "Can't load PDL::Core module (required by Flux::World)");

 PDL = INT2PTR(Core*, SvIV( CoreSV ));  /* Core* value */
 if (PDL->Version != PDL_CORE_VERSION)
    Perl_croak(aTHX_ "Flux::World needs to be recompiled against the newly installed PDL");
