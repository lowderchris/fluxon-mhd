...
filename = f'{args.dat_dir}/{run_name}/cr{CR}/wind/radial_wind.dat'
...
plt.tight_layout()
plt.savefig(f"{args.dat_dir}/{run_name}/cr{CR}/wind/vel_hist_{len(vel1)}.png")
# plt.show()
plt.close(fig)
...
for interp in which_interp:
    ...
    sc00 = ax0.scatter(ph0_clean, th0_clean, c= vel0_clean, s=v0, alpha=0.5, label='V(1.0R$_\odot$)'  , cmap="winter",  )
    sc01 = ax0.scatter(ph1_clean, th1_clean, c= vel1_clean, s=v1, alpha=0.5, label='V(21.5R$_\odot$)' , cmap="autumn",  marker='s')
    ...
    ax0.scatter(ph0b, th0b, c= outlier_V0_scaled, alpha=0.6, label='V(1.0R$_\odot$)', cmap="winter", marker='X',  s=50, edgecolors='k')
    ax0.scatter(ph1b, th1b, c= outlier_V1_scaled, alpha=0.6, label='V(1.5R$_\odot$)', cmap="autumn", marker='X',  s=50, edgecolors='k')
    ax1.scatter(ph0b, th0b, c= outlier_V0_scaled, alpha=0.95, label='V(1.0R$_\odot$)', cmap="winter", marker='X', s=50, edgecolors='k')
    ax2.scatter(ph1b, th1b, c= outlier_V1_scaled, alpha=0.95, label='V(1.5R$_\odot$)', cmap="autumn", marker='X', s=50, edgecolors='k')
    ...
    directory = f"{args.dat_dir}/{run_name}/imgs/wind_maps"
    ...
    def plot_three(grid_x, grid_y, grid_z, save_path):
        ...
        axarr[0].imshow(grid_z, cmap ="autumn", extent=extent, interpolation='None', origin='lower', aspect='auto')
        axarr[1].set_title("contourf")
        axarr[1].contourf(grid_x, grid_y, grid_z, zorder=0, alpha=1, cmap="autumn")
        axarr[2].set_title("hexbin")
        axarr[2].hexbin(grid_x.flatten(), grid_y.flatten(), C=grid_z.flatten(), gridsize=20, cmap='autumn')
        plt.tight_layout()
